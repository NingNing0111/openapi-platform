...
import me.pgthinker.model.dto.user.UserAddRequest;
import me.pgthinker.model.dto.user.UserLoginRequest;
import me.pgthinker.model.dto.user.UserQueryRequest;
import me.pgthinker.model.dto.user.UserRegisterRequest;
import me.pgthinker.model.dto.user.UserUpdateMyRequest;
import me.pgthinker.model.dto.user.UserUpdateRequest;
...
import static me.pgthinker.backend.service.impl.UserServiceImpl.SALT;
...
public class UserController {
    ...
    public void initAdminUser(){
        ...
        wp.eq("userAccount","admin");
        User one = userService.getOne(wp);
        if(ObjectUtils.isEmpty(one)){
            UserRegisterRequest urr = new UserRegisterRequest();
            urr.setUserAccount("admin");
            urr.setUserPassword("123123123");
            urr.setCheckPassword("123123123");
            BaseResponse<Long> resp = userRegister(urr);
            Long userId = resp.getData();
            User storeUser = userService.getById(userId);
            storeUser.setUserRole(UserRoleEnum.ADMIN.getValue());
            userService.updateById(storeUser);
        }
    }
